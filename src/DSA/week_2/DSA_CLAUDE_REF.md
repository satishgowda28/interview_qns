...
5. **Tag it honestly:**
   - Cold ✅ — reasoned it out live, no prior memory of the approach, no video watched first
   - Guided 🎥 — watched the video/approach first, then implemented independently without copying code
   - Recognized 🟢 — remembered the approach from past exposure, executed correctly
   - Almost 🔁 — needed a hint or got partway then stuck
   - Nope ❌ — needed the full solution
6. Revisit all 🔁 and ❌ on the last day of each week.

**Note (Week 1):** Satish's actual workflow is watching the approach video first, then implementing independently without looking at code. This is tagged as Guided 🎥 — it's a legitimate way to learn the pattern, but it's a different skill than deriving the approach cold. The real test of pattern intuition is whether, by later weeks, problems can be attempted Cold (no video first) before falling back to Guided. Track the ratio of Cold vs Guided over time — that ratio is the actual signal of growing confidence, more than raw problems solved.
...
|------|-------|-------|--------|
| 1 | Arrays & Hashmaps | Foundation | ✅ Complete |
| 2 | Two Pointers | Foundation | 🟡 In Progress |
...
- Day 1 (Contains Duplicate, Valid Anagram, Two Sum): All three solved cold ✅. Strong start — Set/Map instinct already present.
- Day 2 (Group Anagrams, Top K Frequent Elements): Both solved cold ✅. Medium step-up handled cleanly — no slowdown from Easy to Medium.
- Day 3 (Product of Array Except Self, Valid Sudoku): Brute force cold ✅, optimal approach Guided 🎥. Prefix/suffix trick needed video — not yet internalized. Flag to revisit prefix pattern when it reappears in later weeks.
- Day 4 (Longest Consecutive Sequence): Warm 🔁 — HashSet direction was clear, sequence-start trick (only count from where n-1 doesn't exist) needed a nudge. Revisit on Day 5.
...
## Week 2 — Two Pointers
**Goal:** Recognise when a sorted array + two indices beats a HashMap. Understand opposite-ends vs slow/fast pointer as two distinct sub-patterns.

### Key Resources
- 📺 Neetcode Two Pointers playlist: https://www.youtube.com/playlist?list=PLALUz6Z8Un2ew_yN3UAce8bOA25P5kaUl
- 📖 Two Pointers pattern explained (Neetcode): https://neetcode.io/courses/dsa-for-beginners/2
...
// Slow/fast pointer — detect cycle or partition
let slow = 0, fast = 0;
...
- Sorted array + find pair/triplet that meets a condition → opposite ends
- Remove/move elements in-place → slow/fast
- Palindrome check → opposite ends moving inward
- "Avoid O(n²)" on a sorted input → two pointers over nested loop
...
**Day 1** — Warm up + opposite ends intro
- Solve: Valid Palindrome (#125), Two Sum II (#167)

**Day 2** — Core medium problems
- Solve: 3Sum (#15), Container With Most Water (#11)

**Day 3** — The hard one
- Solve: Trapping Rain Water (#42)

**Day 4** — Revisit
- Re-solve any 🔁 or ❌ from Days 1–3
- Write both pointer templates from memory

### Week 2 Notes (fill as you go)
- Problems I looked up:
- Patterns that clicked:
- Patterns still unclear:
